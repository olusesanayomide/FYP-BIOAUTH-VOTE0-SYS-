...
const AuditTrailPage = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      if (!resp.success || !resp.data) {
        setError(resp.error || "Failed to load audit logs");
        return;
      }
  ...
  const resourceOptions = useMemo(
    () =>
      Array.from(
        new Set(logs.map((l) => l.resource_type).filter((v): v is string => Boolean(v))),
      ).sort(),
    [logs],
  );
  ...
  const adminOptions = useMemo(
    () =>
      Array.from(
        new Set(logs.map((l) => l.admin_name).filter((v): v is string => Boolean(v))),
      ).sort(),
    [logs],
  );
  ...
  const filteredLogs = useMemo(() => {
    ...
    if (adminFilter !== "all") {
      result = result.filter((l) => (l.admin_name || "System") === adminFilter);
    }
  ...
  }, [logs, dateFilter, statusFilter, actionFilter, resourceFilter, adminFilter, searchQuery]);
  ...
  return (
    <DashboardLayout breadcrumb={["Audit Trail"]}>
      <motion.div
      ...
      >
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={fetchLogs}
              className="inline-flex items-center gap-2 px-3 py-2 rounded-lg text-xs border border-border/50 text-muted-foreground hover:text-foreground hover:bg-muted/40 transition-all"
              disabled={isLoading}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-3">
          ...
          <select
            ...
            className="h-10 px-3 rounded-lg bg-muted/40 border border-border/50 text-foreground text-xs focus:outline-none focus:border-primary/50 appearance-none xl:col-span-2"
          >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border/20">
                  {["Time", "Action", "Status", "Resource", "Actor", "IP", "Actions"].map((h) => (
                    <th
                      key={h}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-foreground">Audit Event Details</h3>
                <button
                  onClick={() => setSelectedLog(null)}
              ...
              <div className="space-y-3 text-sm">
                {[
                  ...
                  ["Actor", selectedLog.admin_name || "System"],
